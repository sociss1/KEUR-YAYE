...
import { ArrowDownRight, Check, LockKeyhole, Minus, Plus, ShoppingBag, Trash2 } from 'lucide-react';
...
type Product={id:number;name:string;price:number;category:string;note:string;tone:string;featured:boolean};
...
const fallback:Product[]=[
  {id:1,name:'Oud El Malick',price:17000,category:'homme',note:'Oud puissant et boisé, sillage affirmé.',tone:'amber',featured:true},
  {id:2,name:'Rose de Yaye',price:15000,category:'femme',note:'Rose épanouie relevée de notes ambrées.',tone:'rose',featured:false},
  {id:3,name:'Nuit Dorée',price:19000,category:'unisexe',note:'Vanille, musc et une pointe de safran.',tone:'gold',featured:false},
  {id:4,name:'Fleur de Thiès',price:14000,category:'femme',note:'Fleurs blanches, fraîches et lumineuses.',tone:'rose',featured:false},
  {id:5,name:'Sillage Royal',price:21000,category:'homme',note:'Cuir et épices pour une présence marquée.',tone:'amber',featured:false},
];
const money=(n:number)=>`${new Intl.NumberFormat('fr-FR').format(n)} F`;
function Bottle({tone='gold'}:{tone?:string}){return <div className={`bottle bottle-${tone}`} aria-hidden="true"><span className="bottle-cap"/><span className="bottle-neck"/><span className="bottle-glass"><span className="bottle-label">KY</span></span></div>}
...
export default function Storefront(){
  ...
  const [orders,setOrders]=useState<Order[]>([]),[form,setForm]=useState({name:'',price:'',category:'homme',note:'',tone:'gold'});
  ...
  async function addProduct(e:React.FormEvent){e.preventDefault();const r=await fetch('/api/store',{method:'POST',headers:adminHeaders,body:JSON.stringify({action:'product',...form})});if(r.ok){setForm({name:'',price:'',category:'homme',note:'',tone:'gold'});await loadProducts()}}
  ...
  return <main>
    <header className="site-header">
      <a href="#top" className="brand" aria-label="KEUR YAYE, accueil"><span className="monogram">KY</span><span>KEUR <em>YAYE</em></span></a>
      <nav aria-label="Navigation principale"><a href="#collection">Collection</a><a href="#histoire">Notre maison</a></nav>
      <Button className="bag-button" variant="ghost" size="icon" aria-label={`Ouvrir le panier, ${count} article(s)`} onClick={()=>setCartOpen(true)}><ShoppingBag size={18}/>{count>0&&<b>{count}</b>}</Button>
    </header>
    ...
    <section id="collection" className="collection">
      ...
      <div className="filters" aria-label="Filtrer la collection">{[['all','Tout voir'],['homme','Homme'],['femme','Femme'],['unisexe','Unisexe']].map(([key,label])=><button key={key} className={filter===key?'active':''} onClick={()=>setFilter(key)}>{label}</button>)}</div>
      <div className="product-grid">{products.filter(p=>filter==='all'||p.category===filter).map((p,index)=><article className="product-card" key={p.id}><div className="product-visual"><span className="product-number">{String(index+1).padStart(2,'0')}</span>{p.featured&&<span className="signature-chip">Signature</span>}<Bottle tone={p.tone}/></div><div className="product-details"><p>{p.category} · {p.note}</p><h3>{p.name}</h3><footer><strong>{money(p.price)}</strong><Button variant="ghost" onClick={()=>add(p.id)}>Ajouter <Plus size={14}/></Button></footer></div></article>)}</div>
    </section>
