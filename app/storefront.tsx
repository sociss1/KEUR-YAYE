...
import { ArrowDownRight, Check, ImageIcon, LockKeyhole, Minus, Plus, ShoppingBag, Trash2, Upload } from 'lucide-react';
...
export default function Storefront(){
  ...
  async function uploadImage(id:number,file:File){const data=new FormData();data.set('image',file);const r=await fetch(`/api/images/${id}`,{method:'POST',headers:{'x-admin-password':password},body:data});if(!r.ok){const e=await r.json().catch(()=>({error:'Échec du téléversement'}));setAdminError(e.error);return}setAdminError('');await loadProducts()}
